/************************************************************
 * PROYECTO WEARABLE IoT – GRUPO CHARLIE
 *
 * Pulsera IoT basada en ESP32 con:
 *  - Medición de pasos (MPU6050)
 *  - Temperatura infrarroja (MLX90614)
 *  - Pantalla LCD GC9A01
 *  - Servidor web embebido vía Wi-Fi
 *  - Interfaz mediante joystick y buzzer
 *
 * El código se divide en bloques funcionales
 * claramente separados para facilitar su
...
// ===================== NTP / ZONA HORARIA =====================
// Servidores NTP para sincronización de hora
const char* NTP_SERVER_1 = "pool.ntp.org";
const char* NTP_SERVER_2 = "time.google.com";

// Configuración de zona horaria (Europa Central)
const char* TZ_INFO = "CET-1CEST,M3.5.0/2,M10.5.0/3";

// ===================== JOYSTICK (ADC como botón) =====================
// El joystick se lee como un botón analógico
#define PIN_JOYSTICK_ADC 32
#define BTN_PRESSED_TH   3800     // pulsado si ADC < umbral
#define BTN_DEBOUNCE_MS  60       // Antirrebote
#define BTN_LONG_MS      5000     // Pulsación larga (Wi-Fi)

// ===================== BUZZER =====================
// Variables para control de alarma sonora
bool alarmEnabled = false;
unsigned long lastBeep = 0;
bool buzzerState = false;
#define BEEP_INTERVAL_MS 500      // Intervalo ON/OFF del buzzer
...
#define TEMP_UPDATE_MS 1000       // Lectura cada 1 s

// Contador de pasos
volatile int stepCount = 0;

// -------- MPU6050 --------
bool mpuAwake = true;             // Estado activo/sleep
unsigned long lastStepSample = 0;
unsigned long lastStepTime = 0;
bool stepPossible = false;

#define STEP_SAMPLE_MS 50       // Periodo de muestreo
#define STEP_DEBOUNCE_MS 300      // Antirrebote de pasos


// ===================== LCD =====================
Adafruit_GC9A01A tft(TFT_CS, TFT_DC, TFT_RST);

// ==== control anti-parpadeo ====
bool screenDirty = true;      // fuerza un pintado inicial
int lastScreen = -1;          // para detectar cambio de pantalla
unsigned long lastPartial = 0;
...
// Variables para detección estable de pulsaciones
...
// Desconexión Wi-Fi para ahorro energético
void disconnectWiFi() {
...
//                      BUZZER
// =========================================================
// Genera pitido intermitente cuando la alarma está activa
void updateAlarm() {
  if (!alarmEnabled) {
    noTone(PIN_BUZZER);
    buzzerState = false;
...
void nextScreen() {
  ...
  screenDirty = true;   // redibujado completo SOLO al cambiar pantalla
}

// Gestión de pulsación corta / larga
void handleButton() {
  ...
  if (btnStable != btnLastStable) {
    ...
    if (btnStable) {
      ...
      if (!longFired) {
        nextScreen(); // pulsación corta -> siguiente pantalla
      }
    }
  }
  ...
  if (btnStable && !longFired && (now - btnPressStart >= BTN_LONG_MS)) {
    ...
    if (wifiOn) {
      ...
      if (wifiOn) syncTimeWithNTP();
    }

    screenDirty = true; // refrescar plantilla para ver WiFi ON/OFF
  }
}
...
//                     WEB
// =========================================================
// Genera la página HTML que se sirve al cliente
// Incluye CSS y JavaScript embebido para evitar archivos externos
String makeHTMLPage() {
  String html;
  ...
  html += "button{padding:10px 14px;border-radius:10px;border:1px solid #ccc;background:#f5f5f5;margin-right:8px;margin-top:6px}";
  ...
  // ===== ESTADO MPU =====
  html += "<p class='big'><b>MPU:</b> <span id='mpu'>--</span></p>";
  html += "<button onclick=\"setMPU(1)\">MPU ON</button>";
  html += "<button onclick=\"setMPU(0)\">MPU SLEEP</button>";
  ...
  // ================== JAVASCRIPT ==================
  // JavaScript para refresco periódico y control remoto
  html += "<script>";

  // --- Refresh periódico de datos vía API ---
  ...
  html += "  document.getElementById('mpu').textContent = j.mpu ? 'ON' : 'SLEEP';";
  ...
  // --- CONTROL MPU ---
  html += "async function setMPU(state){";
  html += "  await fetch(window.location.origin + '/mpu?state=' + state, {method:'POST'});";
  html += "}";
...
// Atiende la raíz del servidor web
void handleRoot() {
  server.send(200, "text/html; charset=utf-8", makeHTMLPage());
}

// Endpoint para activar/desactivar alarma
void handleAlarm() {
...
// Endpoint para poner el MPU6050 en sleep o wake
void handleMpu() {
  if (!server.hasArg("state")) {
    server.send(400, "text/plain", "Missing state");
    return;
  ...
  screenDirty = true;  // refrescar LCD
  server.send(200, "text/plain", mpuAwake ? "MPU ON" : "MPU SLEEP");
}

// Endpoint JSON con estado del sistema
void handleApi() {
  String json = "{";
  ...
  json += "\"mpu\":" + String(mpuAwake ? "true" : "false") + ",";
...
// Inicializa rutas del servidor web
void setupWebServer() {
  server.on("/", HTTP_GET, handleRoot);
  server.on("/alarm", HTTP_POST, handleAlarm);
  server.on("/mpu", HTTP_POST, handleMpu);
...
// Actualiza la temperatura solo cuando la pantalla MLX está activa
...
// Algoritmo de detección de pasos basado en aceleración
void updateStepCounter() {
  if (!mpuAwake) return;   // ⛔ MPU dormido → no leer nada
  ...
  // Fase de caída → posible paso
  if (A < 0.9) stepPossible = true;

  // Pico de aceleración → paso válido
...
// Pone el MPU6050 en modo bajo consumo
void mpuSleep() {
  Wire.beginTransmission(ADDR_MPU6050);
  Wire.write(0x6B);      // PWR_MGMT_1
  Wire.write(0x40);      // bit SLEEP = 1
...
// Pinta la “plantilla” completa (solo cuando screenDirty)
void drawScreen() {
  ...
  // zona de valor (se actualiza parcial)
  // (dejamos un hueco negro para escribir valores)
  tft.fillRect(20, 125, 200, 45, GC9A01A_BLACK);
...
// Actualiza solo el valor de la pantalla actual (sin fillScreen)
void updateScreen() {
  // Redibujado completo solo cuando cambias
  if (screenDirty || lastScreen != currentScreen) {
    lastScreen = currentScreen;
    screenDirty = false;
    drawScreen();
    // fuerza una actualización inmediata del valor
    lastPartial = 0;
  }

  // actualizaciones parciales (1 vez por segundo)
  if (millis() - lastPartial < 1000) return;
  lastPartial = millis();

  // “borramos” solo el área del valor
  tft.fillRect(20, 125, 200, 45, GC9A01A_BLACK);


  if (currentScreen == SCREEN_MPU) {
    // Pasos (grande)
    tft.setTextSize(2);
  ...
  else if (currentScreen == SCREEN_MLX) {
    tft.setTextSize(2);
